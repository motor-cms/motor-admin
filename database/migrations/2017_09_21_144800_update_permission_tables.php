...
class UpdatePermissionTables extends Migration
{
    ...
    public function up()
    {
        ...
        foreach (DB::table('user_has_roles')->get() as $ur) {
            $u = \Motor\Admin\Models\User::find($ur->user_id);
            $r = \Motor\Admin\Models\Role::find($ur->role_id);
            $u->assignRole($r->name);
        }

        foreach (DB::table('user_has_permissions')->get() as $up) {
            $u = \Motor\Admin\Models\User::find($up->user_id);
            $p = \Motor\Admin\Models\Permission::find($up->permission_id);
